class ReviewsController < ApplicationController
  ...
  def create_with_type
    ...
    when "final"
      @answers = Answer.where(active: true, review_completed:false, assignment_id: params[:id], is_blank_submission: false).where("user_id NOT in (?) and answers.id NOT in (?)", current_user.id, @reviewed_answers)
    else
