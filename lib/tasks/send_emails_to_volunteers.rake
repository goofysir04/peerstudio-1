		...
		if !assignments.nil? 
			assignments.each do |assign|
				...
				if !thirty_min_ans.empty?
					...
					have_to_review_still = TriggerAction.where(["assignment_id = ? and `trigger` = ? and count > ? and last_email_time IS NULL or last_email_time < ?", assign.id, "review_required", 0, 12.hours.ago]).order(random_function).limit(5)
					if have_to_review_still.empty? #nil didn't work, so used empty
						...
						volunteers = TriggerAction.where(["assignment_id = ? and `trigger` = ? and count > ? and last_email_time IS NULL or last_email_time < ?", assign.id, "email_count", 0, 12.hours.ago]).order(random_function).limit(5)
						...
						have_to_review_still.each do |h|
							...
							if !h.user.nil? and !h.user.opted_out_help_email?
