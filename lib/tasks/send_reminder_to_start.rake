		...
		@assignments.each do |a|
			...
			users_that_havent_started.each do |enrollment|
				...
				if enrollment.user.experimental_condition == "waitlist"
					...
					# ReviewMailer.delay.send_waitlist_reminder(enrollment.user, a)
				else
					logger.info "Sending an activated reminder"
					# ReviewMailer.delay.send_reminder_to_start(enrollment.user, a)
				end
			end
		end
	end
		...
		a = Assignment.find(9)
			...
			users_that_havent_started.each do |user|
				...
				if user.experimental_condition == "waitlist"
					logger.info "Sending a wait list reminder"
					ReviewMailer.delay.send_waitlist_reminder(user, a)
				else
					logger.info "Sending an activated reminder"
					ReviewMailer.delay.send_reminder_to_start(user, a)
				end
			end
	end
end
